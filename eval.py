...
DetectionResult = namedtuple(
    'DetectionResult',
    ['box', 'score', 'class_id', 'posterior', 'mask', 'feature_fastrcnn_pooled'])
SecondDetectionResult = namedtuple(
    'SecondDetectionResult',
    ['box', 'score', 'class_id', 'posterior', 'mask', 'second_class_id', 'second_posterior', 'feature_fastrcnn_pooled'])
"""
box: 4 float
score: float
...
def detect_one_image(img, model_func, *args):
    ...
    if config.USE_SECOND_HEAD:
        if config.EXTRACT_FEATURES:
            boxes, probs, labels, posteriors, second_labels, second_posteriors, masks, features = model_func(resized_img, *args)
            masks = [masks]
        else:
            boxes, probs, labels, posteriors, second_labels, second_posteriors, *masks = model_func(resized_img)
            features = [None for _ in range(labels.size)]
    else:
        if config.EXTRACT_FEATURES:
          boxes, probs, labels, posteriors, masks, features = model_func(resized_img, *args)
          masks = [masks]
        else:
    ...
    if config.USE_SECOND_HEAD:
        ...
        #print (len(labels))
        #print (len(features))
        results = [SecondDetectionResult(*args) for args in zip(boxes, probs, labels, posteriors, masks,
                                                                second_labels, second_posteriors, features)]
    else:
        results = [DetectionResult(*args) for args in zip(boxes, probs, labels, posteriors, masks, features)]
    return results

...
if config.CATEGORY_AGNOSTIC:
    config.NUM_CLASS = 1 + 1
else:
    config.NUM_CLASS = COCO_NUM_CATEGORY + 1
config.SECOND_NUM_CLASS = COCO_NUM_CATEGORY + 1
    ...
    def create(self, cat_ids, cat_names):
        ...
        if config.CATEGORY_AGNOSTIC:
            ...
            self.category_id_to_class_id = {
                v: 1 for i, v in enumerate(cat_ids)}
        ...
        if config.USE_SECOND_HEAD:
            self.second_cat_names = cat_names
            self.second_class_names = ['BG'] + self.second_cat_names
            # background has class id of 0
            self.second_category_id_to_class_id = {
                v: i + 1 for i, v in enumerate(cat_ids)}
            self.second_class_id_to_category_id = {
                v: k for k, v in self.second_category_id_to_class_id.items()}
            config.SECOND_CLASS_NAMES = self.second_class_names
...
class COCODetection(object):
    def __init__(self, basedir, name):
        ...
        if not COCOMeta.valid():
            ...
            if not config.CATEGORY_AGNOSTIC:
                assert COCOMeta.cat_names == cat_names
            if config.USE_SECOND_HEAD:
                assert COCOMeta.second_cat_names == cat_names
    ...
    def _add_detection_gt(self, img, add_mask):
        ...
        if config.USE_SECOND_HEAD:
            second_cls = np.asarray([COCOMeta.second_category_id_to_class_id[obj['category_id']]
                for obj in valid_objs], dtype='int32')  # (n,)
            img['second_class'] = second_cls
